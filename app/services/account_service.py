...
class AccountService:
    """Read Instagram account rows shared with the dashboard database."""
    ...
    async def get_account_by_instagram_user_id(self, instagram_user_id: str) -> InstagramAccount | None:
        async with self._session_factory() as session:
            return await self.get_by_instagram_user_id(session, instagram_user_id)
    ...
    async def get_by_username(self, username: str) -> InstagramAccount | None:
        ...
        async with self._session_factory() as session:
            result = await session.execute(
                select(InstagramAccount).where(
                    func.lower(InstagramAccount.username) == normalized,
                    InstagramAccount.is_active.is_(True),
                )
            )
            return result.scalar_one_or_none()

    async def list_active_accounts(self) -> list[InstagramAccount]:
        async with self._session_factory() as session:
            result = await session.execute(
    ...
    async def list_agent_profiles(self) -> list[AgentProfileConfig]:
        accounts = await self.list_active_accounts()
        return [AgentProfileConfig.from_account(account) for account in accounts]

    async def resolve_account(self, instagram_user_id: str) -> InstagramAccount | None:
        """Load active account by Instagram user ID without env bootstrap."""
        return await self.get_account_by_instagram_user_id(instagram_user_id)
    ...
    async def _bootstrap_default_account(self, session: AsyncSession) -> InstagramAccount | None:
        ...
        if account:
            ...
            if not account.ai_api_key and self._settings.gemini_api_key:
                account.ai_api_key = self._settings.gemini_api_key
            await session.commit()
            return account
        ...
        account = InstagramAccount(
            ...
            ai_provider="gemini",
            ai_api_key=self._settings.gemini_api_key,
        ...
        session.add(
            Knowledge(
                ...
                category="about",
                title="About",
                content="Configure this knowledge entry in the dashboard.",
                sort_order=0,
            )
        )
        await session.commit()

...
# Fixed reply for voice notes / photos / stickers / video — no Gemini.
...
class DmTaskHandler(BaseTaskHandler):
    async def handle(self, ctx: HandlerContext, task: Task, event: Event) -> None:
        ...
        if data.unsupported_media or (
            data.attachment_types and not str(data.text or "").strip()
        ):
            reply_text = UNSUPPORTED_MEDIA_REPLY
            ...
            async with ctx.session_factory() as session:
                ...
                await repo.store_message(
                    ...
                    text=f"[unsupported:{','.join(data.attachment_types) or 'media'}]",
                    direction="incoming",
            ...
            log_event(
                logger,
                logging.INFO,
                "unsupported_media_reply_sent",
                task_id=task.id,
                event_id=event.event_id,
                recipient_id=recipient,
                attachment_types=list(data.attachment_types),
            )
            return
